...
- (#2) Hive의 Binary 데이터 추출 오류 - 문자형으로 표현할 수 없는 데이터에 추출 시 데이터 오변환
- (#39) 수집/적재 방식 Refactoring
- (#8) 원천 RDBMS 접속 정보 이중화
- (#7) 단일 테이블의 다중 쓰레드/커넥션 적재
- (#9) 실시간 변경분 수집/적재

## OpenAPI 수집/적재

- (#4) 응답 메시지 오류 처리 방식 고도화

## 상황판

- (#5) 상황판 기능 정리 및 통합
- (#10) 해경 관련 화면 요소 제거 및 대치

## 기타

- (#3) 제품명에 따른 디렉토리/파일명 등 수정
- (#14) 배치 수행 및 결과 정리 Refactoring
- (#11,#12) 메타 수집 및 검증 기능
- (#13) Hive의 데이터 추출(RDBMS) 절차 정리
- (#15) 배치 수행 통합 및 의존성 기능 추가 - 복수의 수집된 데이터들을 가공처리하여 RDBMS적재/파일추출 등
- (#18) SequenceFile 이외의 다양한 형식의 적재 방식 추가
- (#8) 전체 테이블 적재 이외의 다양한 추가/변동 데이터 적재 지원
- (#17) 수집기의 분산 처리
- (#19) 배치 수행 실시간 조회 및 결과 조회(Web)
- (#20) 적재 상태 조회 API(Command Line)
- (#21) JDBC2JSON의 Cursor 기능 추가
- (#18) 문서 작성 및 정리
